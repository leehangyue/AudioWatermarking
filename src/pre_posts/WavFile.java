...
// https://sourceforge.net/p/cmusphinx/discussion/sphinx4/thread/3238272f/20d8/attachment/WavFile.java
...
public class WavFile
{
	...
	private double upLim;               // Upper limit of double for float <-> int conversion (clipping)
	...
	// Buffering
	private byte[] buffer = new byte[BUFFER_SIZE];;					// Local buffer used for IO
	...
	//Constructor for new wav file
	public WavFile(File file, int numChannels, long numFrames, int validBits, long sampleRate) throws IOException, WavFileException
	{
		this.file = file;
		...
		this.upLim = 1.0 - 1.0 / (1L<<(8*bytesPerSample));
		...
		// Write Format Chunk
		this.oStream.write(this.buffer, 0, 24);
	...
	//Constructor for opening existing wav file
	public WavFile(File file) throws IOException, WavFileException
	{
		this.file = file;
		...
		while (true)
		{
			...
			if (chunkID == FMT_CHUNK_ID)
			{
				...
				if (this.numChannels == 0) throw new WavFileException("Number of channels specified in header is equal to zero");
				if (this.blockAlign == 0) throw new WavFileException("Block Align specified in header is equal to zero");
				if (this.validBits < 2) throw new WavFileException("Valid Bits specified in header is less than 2");
				if (this.validBits > 64) throw new WavFileException("Valid Bits specified in header is greater than 64, this is greater than a long can hold");
				...
				numChunkBytes -= 16;
				if (numChunkBytes > 0) this.iStream.skip(numChunkBytes);
			}
			else if (chunkID == DATA_CHUNK_ID)
			{
				...
				if (chunkSize % this.blockAlign != 0) throw new WavFileException("Data Chunk size is not multiple of Block Align");
	...
	public int writeFrames(double[] sampleBuffer, int offset, int numFramesToWrite) throws IOException, WavFileException
	{
		...
		for (int f=0 ; f<numFramesToWrite ; f++)
		{
			...
			for (int c=0 ; c<numChannels ; c++)
			{
				temp = sampleBuffer[offset];
				if(temp < -upLim) temp = -upLim;
				if(temp > upLim) temp = upLim;
				writeSample((long) (floatScale * (floatOffset + temp)));
				offset ++;
			}
	...
	public static void main(String[] args)
	{
		...
		try
		{
			...
			{
				WavFile readWavFile = new WavFile(new File(filename));
				readWavFile.display();
				...
				WavFile writeWavFile = new WavFile(new File("out.wav"), numChannels, numFrames, validBits, sampleRate);
...
//				long[] buffer = new long[BUF_SIZE * numChannels];
			...
			WavFile writeWavFile = new WavFile(new File("out2.wav"), 1, 10, 23, 44100);
